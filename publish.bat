...
echo ========================================
python -c "import re; content = open('pyproject.toml').read(); name = re.search(r'name = \"([^\"]+)\"', content); version = re.search(r'version = \"([^\"]+)\"', content); print(f'  Name:    {name.group(1)}') if name else None; print(f'  Version: {version.group(1)}') if version else None; print(f'  Repo:    https://github.com/PayloadShield/FastAPIPS')"
echo ========================================
echo.
